    ...
    export default {
        ...
        methods: {
            getUserFiles(){
                ...
                var mainCollection = {};
                $.when(
                    firestore.collection("UserImages").get().then((querySnapshot) => {
                    querySnapshot.forEach((doc) => {
                        if (doc.exists) {
                            mainCollection[doc.id] =  doc.data();
                        }
                    });
                })
                ).then(function () {
                    that.allFiles =mainCollection; 
                });
            },
            selectFile() {
            ...
            alertConfirm(filename, key) {
                ...
                swal.fire({
                    ...
                    if (result.value) {
                        ...
                        firestore.collection("UserImages").doc(key).delete().then(function() {
                            swal.fire({
                                title: "Success!",
                ...
                this.uploadTask.on('state_changed', sp => {
                    ...
                    () => {
                        this.uploadTask.snapshot.ref.getDownloadURL().then(downloadURL => {
                            ...
                            firestore.collection("UserImages").add({
